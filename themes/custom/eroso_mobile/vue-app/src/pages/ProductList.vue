    ...
    <main class="pt-16 pb-24 px-4 lg:ml-64">
      ...
      <div v-show="!loading && !imageSearching && displayProducts.length === 0" class="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h3 class="text-base font-semibold text-gray-900">
          {{ imageSearchActive ? 'Aucun produit correspondant' : 'Aucun produit disponible' }}
        </h3>
        <p class="text-sm text-gray-500 mt-1">
          {{ imageSearchActive ? 'Essayez une autre photo ou effacez la recherche IA.' : 'Aucun produit avec le statut "dispo" trouvé.' }}
        </p>
      </div>

      <!-- Product List -->
      <div v-show="displayProducts.length > 0" class="grid grid-cols-1 gap-4">
        <div v-for="product in displayProducts" :key="product.nid" @click="router.push(`/product/${product.nid}`)" class="bg-white rounded-2xl shadow-sm p-4 border border-gray-100 transition-all hover:shadow-md active:scale-[0.98] cursor-pointer">
      ...
      <div ref="loadMoreTrigger" class="flex justify-center py-8 min-h-[100px] items-center">
        <div v-show="(loading || imageSearching) && displayProducts.length > 0" class="w-8 h-8 border-3 border-blue-600 border-t-transparent rounded-full animate-spin"></div>
